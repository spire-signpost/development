...
app.post('/notes', authenticate, (req, res) => { // add authentication to route
...
app.get('/notes/:id', authenticate, (req, res) => { // add authentication to route
  ...
  // test find for GET route with params - e.g. user ID and author - private route and query
  Note.findOne({
    _id: params_id,
    author: req.user._id
  }).then((note) => {
...
app.delete('/notes/:id', authenticate, (req, res) => { // add authentication to route
  // get params ID from req
  var params_id = req.params.id;
  console.log(params_id);
...
app.patch('/notes/:id', authenticate, (req, res) => { // add authentication to route
  // get params ID from req
  var params_id = req.params.id;
  console.log(params_id);
  ...
  Note.findOneAndUpdate({
    _id: params_id,
    author: req.user._id
  }, {$set: body}, {new: true}).then((note) => { // MongoDB update - set object to body, and return the new doc object - new: true (mongoose naming for returnOriginal)
...
// 3. DELETE route for user logout - /users/me/token
